...
router.put("/resetFirstPassword", async (req, res) => {
...
router.get("/viewNotifications", auth, async (req, res) => {
  try {
...
router.put("/viewAttendence", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["-password", "-_id"]);
...
router.put("/viewMissingDays", auth, async (req, res) => {
  try {
    var { month, year } = req.body;
...
router.put("/viewMissinghrs", auth, async (req, res) => {
  try {
    var { month, year } = req.body;
    // var { month, year } = req.body;
...
router.put("/helpsalary", auth, async (req, res) => {
  try {
    var { month, year } = req.body;
    // var { month, year } = req.body;
...
router.put("/updatemyProfile", auth, async (req, res) => {
  try {
    let {
      name,
      email,
...
/////////////////////////////////////////////////////MARTHA/////////////////////////////////////////////////////
/////////////////////////////////////////////////////HR/////////////////////////////////////////////////////
router.get("/viewLocations", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["notifications", "-_id"]);
...
router.put("/locations/:locname", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
router.delete("/locations/:locname", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
////////////////////////////////////////////
router.get("/viewFaculties", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["notifications", "-_id"]);
...
router.put("/faculties/:name", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
router.delete("/faculties/:name", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
///////////////////////////////
router.get("/viewDepartments", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["notifications", "-_id"]);
...
router.put("/departments/:name", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
router.delete("/departments/:name", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
    ...
    const head1 = existingdep.head;
    if (head1) {
      const findhead = await members.findOne({ _id: head1 });
      findhead.role = "Course Instructor";
      await findhead.save();
    }
...
///////////////////Courses
router.get("/viewCourses", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["notifications", "-_id"]);
...
router.get("/viewmember/:id", auth, async (req, res) => {
  try {
    const existingUser = await members
      .findOne({ id: req.id })
      .select(["notifications", "-_id"]);
...
//Need to Modify??
router.put("/members/missesdays", auth, async (req, res) => {
  try {
    const existingUser = await members.findOne({ id: req.id });
    if (!existingUser) {
      return res.json({ msg: "This User doesn't Exist in my Database" });
...
router.put("/members/misseshrs", auth, async (req, res) => {
  try {
    const existingUser = await members.findOne({ id: req.id });
    if (!existingUser) {
      return res.json({ msg: "This User doesn't Exist in my Database" });
...
///Ask Martha??
router.put("/salary/:id", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res
...
router.put("/updatesignout/:id", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
    ...
    if (req.id.toLowerCase() == req.params.id.toLowerCase()) {
      return res.json({
...
router.put("/updatemember/:id", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
    ...
    if (role) {
      ...
      if (role == "HOD") {
        ...
        departments.findOne({ _id: depID }).then((dep) => {
...
router.post("/addfirsthr", async (req, res) => {
  try {
    let deducted_salry;
    let annualLeaveBalance = 2.5;
    let accidentalLeaves = 6;
...
//////////////////////////////
router.put("/updatesignin/:id", auth, async (req, res) => {
  try {
    const mawgood = await members.findOne({ id: req.id });
    if (!mawgood) {
      return res.json({
...
router.put("/assigninstructor", auth, async (req, res) => {
  try {
...
router.delete("/removeinstructorfromcourse", auth, async (req, res) => {
...
router.put("/updateinstructorcourses", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to do this" });
    }
...
router.put("/viewStaffinDepartment", auth, async (req, res) => {
  //needs test
...
router.put("/viewDayoff", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to do this" });
    }
...
router.get("/viewHodRequests", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to do this" });
    }
...
router.put("/acceptLeaveRequest", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to do this" });
    }
...
router.put("/acceptDayoffRequest", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.status(400).json({ msg: "You are not authorized to do this" });
    }
...
router.put("/rejectHodRequest", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to do this" });
    }
...
router.get("/viewdepartmentcoverage", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
...
router.get("/viewdepartmentslots", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.put("/viewcourseslots", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.put("/viewcoursecoverage", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.get("/viewdepartmentcourses", auth, async (req, res) => {
  try {
    if (req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.put("/viewreplacementbyid", auth, async (req, res) => {
  try {
    const hod = await members.findOne({ id: req.id });
    const hod_id = hod._id;
    const { requestid } = req.body;
...
/////////////////////////////////////////FARAH//////////////////////////////////////////////////////////////
/////////////////////////////////////////FARAH//////////////////////////////////////////////////////////////
router.get("/viewstaff/:id", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" && req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.post("/coursecoordinator/:id/:idcourse", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" && req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.post("/assignslot/:id/:idslot", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" &&  req.tokenrole != "HOD" ) {
      return res.json({ msg: "You are not authorized to to this" });
    }
    ...
    const idmember = existinguser._id;
    const idmember2 = existinguser2._id;
    //return res.json({ msg: "testtttttt " });
    if (roleuser=="HR") {
      return res.json({ msg: "The id of must belong to academic member" });
    }
    ...
    const matchfinal = await courses.findOne({
      instructors: idmember,
      id: slotcourseid,
    });
    ...
    if (roleuser=="Course Instructor") {
      ...
      if(!matchfinal){
        return res.json({
          msg:
            " The academic member you entered must be course instructor  to be assigned to this course",
        });
    ...
    if (!match2 && roleuser!="Course Instructor") {
...
router.get("/viewslots/:id", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" && req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.delete("/deleteassigncourse/:id/:idcourse", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" && req.tokenrole != "HOD") {
      return res.status(400).json({ msg: "You are not authorized to to this" });
    }
...
router.get("/viewcoverage", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course Instructor" && req.tokenrole != "HOD") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.get("/viewslotlinkingrequest", auth, async (req, res) => {
  try {
...
router.post("/acceptslotlinking", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({ msg: "You are not authorized to to this" });
    }
    ...
    console.log(checkRequest.weekday);
    ...
    if (mySlots.length != 0) {
      return res.json({
        msg:
          "You can't accept as this sender has already assigned slot on " +
          checkRequest.weekday +
          " at " +
          checkRequest.slottime +
          " slot ",
      });
    }
    ...
    let notify =
      "Your Slot Linking Request to Id " +
      myRecord.id +
      " to teach  " +
      check3.id +
      " On" + 
    ...
    let senderRecord = await members.findOne({ _id: sender });
...
router.post("/rejectslotlinking", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.get("/getCourseSlots", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({
...
router.post("/addslot", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({ msg: "You are not authorized to to this" });
    }
    ...
    if (!slottype) {
      return res.json({ msg: "Please enter slottype" });
    }
    ...
    if (!locname) {
      return res.json({ msg: "Please enter location" });
    }
...
router.delete("/deleteslot", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({ msg: "You are not authorized to to this" });
    }
...
router.post("/updateslot", auth, async (req, res) => {
  try {
    if (req.tokenrole != "Course coordinator") {
      return res.json({ msg: "You are not authorized to to this" });
    }

    //console.log("d5ltayeaaaa")
...
router.post("/sendSlotLinking", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
      return res.json({
...
router.post("/sendChangeDayOff", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
      return res.json({
...
router.post("/sendSlotReplacement", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
      return res.json({
...
router.get("/viewReceivedReplacements", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
      return res.json({
    ...
    console.log("d55aalltttttttttttt ");
    res.send({ myRequests });
  } catch (error) {
    res.json({ error: error.message });
  }
...
router.put("/viewSentRequests/", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
...
router.put("/acceptReplacemant", auth, async (req, res) => {
  try {
    let { requestId } = req.body;
    const check = {
      requestId: requestId,
...
router.put("/rejectReplacemant", auth, async (req, res) => {
  try {
    let { requestId } = req.body;
...
router.post("/sendLeave", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
    ...
    if (leavetype == "Compensation") {
      ...
      try {
        let dd1 = compensationDate.substring(0, 2);
        let mm1 = compensationDate.substring(3, 5);
        let yyyy1 = compensationDate.substring(6, 10);
...
router.delete("/cancelRequest", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
      return res.json({ msg: "An Hr member cannot send a Leave Request " });
    }
...
router.get("/viewMySchedule", auth, async (req, res) => {
  try {
    if (req.tokenrole == "HR") {
